...
#   Contributions 2009-2013 Torbjorn Bjorkman
#
...
#
#   Project adaptation (2026-08-31):
#     - Ported to Python 3 (removed `commands`, `print` statements,
#       `file()`; bytes handling for FINDSYM subprocess).
#     - Replaced shell-out `grep`/`rm` with Python-native file reads,
#       so the script also runs on Windows.
#     - `--no-findsym` now works as a plain flag.
# ***********************************************************
...
parser.add_option("--no-findsym", dest="nofindsym", action="store_true", help="Don't run FINDSYM to find symmetry of the crystal.")
...
def _remove_file(path):
    """Remove a file if it exists (cross-platform replacement for `rm -f`)."""
    try:
        os.remove(path)
    except OSError:
        pass
...
def _titel_lines(path):
    """Read all TITEL lines from a POTCAR/OUTCAR file (Python-native, no grep)."""
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            return [line for line in fh if line.startswith("TITEL")]
    except OSError:
        return []
...
class Cell:
    def __init__(self):
        ...
        self.gamma = None
        self.HMSymb = "'P 1'"
        self.sites = []  # list of (elementname, x, y, z) tuples
...
def ciffilestring(cell):
    ...
    if findsym:
        ...
        p = Popen(["findsym"], stdin=PIPE, stdout=PIPE)
        p.stdin.write(findsymstring.encode("utf-8"))
        p.stdin.flush()
        findsymoutput = p.communicate()[0].decode("utf-8", errors="replace")
        findsymoutlines = findsymoutput.split("\n")
        # Delete log
        _remove_file("findsym.log")
    ...
    if filetype(input_file) == "POSCAR":
        ...
        if options.elements:
            ...
            if vasp5:
                ...
                potcar_lines = _titel_lines("POTCAR")
        ...
        if scale_volume:
            ...
            a = list(map(lambda x: lattice_constant * x, a))
            b = list(map(lambda x: lattice_constant * x, b))
            c = list(map(lambda x: lattice_constant * x, c))
        ...
        atoms = []
        titellines = _titel_lines(filename)
        if len(titellines) == 0:
        ...
        with open(filename, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                if "ions per type =" in line:
                    natoms = [int(s) for s in line.split()[4:]]
                    break
