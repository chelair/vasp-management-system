...
### 智能报告 Report（v0.7.0 重构：分项目报告）
- **以单个项目为报告单位**，后端生成三份产物：结构化数据 `report.json`（schema 版本化，可直接喂大模型）
  + Markdown `report.md` + SVG 图表 `charts/*.svg`，按项目分目录存 `data/reports/`，索引 `data/reports/index.json`
- 报告含 11 个章节：报告元数据 / 项目执行摘要 / 进度总览 / 任务状态详情 / 科学结果分析 /
  巡检与异常汇总 / 资源与集群健康 / 风险分析 / 下一步行动清单 / 面向大模型的上下文 / 附录
...
- **Markdown 与结构化数据一一对应**；图表为纯 Python 生成的 SVG（零第三方依赖），数据同时以数组提供
- 前端：按项目生成 / 一键生成所有项目、报告历史（按项目分组）、章节导航、
  **导出范围可勾选**（如取消风险分析）、查看结构化数据、下载 MD/JSON、
  **导出 HTML（自包含、图表内联）与 PDF（浏览器打印，按勾选章节）**
