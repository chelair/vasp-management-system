...
当前版本（**v0.9.11**，运行在 Linux 生产机）：四个核心模块（总览 / 巡检中心 / 作业管理 / 智能报告）+ SSH 连接配置界面（**v0.9.0 起需登录**：账号/会话 + 登录页 + token 鉴权；**v0.9.2 起按项目归属隔离**：普通用户只能看到自己创建的项目，越权 403，admin 全可见；**v0.9.3 起前端按角色隐藏入口**：普通用户看不到 SSH/全局配置与全局巡检入口，403 只弹轻提示不跳登录）；**v0.9.6 新增自动/定时执行动作系统**（巡检事件 + cron 触发 → 规则 → 冷却/上限/幂等/熔断 → 四个动作：续算/提交/NEB/频率矫正 → 五态审计，前端「自动化」页仅 admin）；作业管理含自研集群节点看板、输入文件草稿与逐项撤销、任务选择弹窗（分组折叠 / 搜索 / 已关闭项目折到最后）、POSCAR 选中原子坐标与固定原子、vasp.lsf 脚本生成与提交前非空检查，巡检结论在作业管理同步显示；
当前版本（**v0.8.8**，运行在 Linux 生产机）：四个核心模块（总览 / 巡检中心 / 作业管理 / 智能报告）+ SSH 连接配置界面；作业管理已含自研集群节点看板、输入文件「同步到远端」、POSCAR 同步/生成 POTCAR/固定原子（Selective Dynamics）、vasp.lsf 八段式脚本生成与提交前非空检查，巡检结论（低精度收敛、力未收敛等）在作业管理同步显示；
