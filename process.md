...
> 生成时间：2026-08-29 · 最近更新：2026-09-13 · 当前版本：v0.7.3（报告看板与巡检详情同版式：自由能台阶 / NEB 能垒 / 分段项目进度；结构优化口径修正为仅独立 opt 任务）
...
1. `git -C D:\Skill\vasp-project-manager-web log --oneline -3` 确认在 v0.7.3；`git status` 应干净（有未提交改动时先看 §7 末尾是否为「待提交」事项）。
