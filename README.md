...
当前版本（**v0.8.9**，运行在 Linux 生产机）：四个核心模块（总览 / 巡检中心 / 作业管理 / 智能报告）+ SSH 连接配置界面；作业管理含自研集群节点看板、输入文件草稿与逐项撤销、任务选择弹窗（分组折叠 / 搜索 / 已关闭项目折到最后）、POSCAR 选中原子坐标与固定原子、vasp.lsf 脚本生成与提交前非空检查，巡检结论在作业管理同步显示；
