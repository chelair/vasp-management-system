...
- v0.9.28（commit `13af4ec`，已推送 origin/main；2026-09-24 用户："ABS/3/con1/OSZICAR 为什么不同步 POSCAR" → 选定 A 方案）：**结构同步的 POSCAR 也跟随最新续算目录（口径与提交/续算/CONTCAR 统一）**。
  ...
  **验证**：隔离 mock 9 项（conN 优先取到 2× 结构 / CONTCAR 同步取 conN / marker 文案 / CIF 两处都写且内容是新的（按晶胞 a=26.8222 断言）/ conN 缺文件回退主目录并记 marker / 无 conN 时用主目录）。
...
- v0.9.23（commit `19e21f3`，已推送 origin/main；2026-09-22 排查"为什么 PATH1 没有下载"时发现并修掉）：**`_list_remote_dirs` 的远端脚本退出码陷阱（会让 NEB 归档在真实远端必然失败）**。
  ...
  验证：脚本级 9 项（"有匹配 / 匹配到非目录 / 一个都没匹配 / 目录不存在"退出码都应为 0；用真实远端的形状做对照：旧写法 rc=1、新写法 rc=0；mock 分支排序）；**真实 HPC 端到端**：对 Ag 的 NEB PATH1/PATH3 跑了一次新逻辑（**远端只读**），PATH3 源目录取 `con17`、PATH1 取 `con3`，各自落地 `files/{INCAR,KPOINTS}` + `files/00..04/{POSCAR,CONTCAR,OUTCAR,OSZICAR}`（每个 16 个映像文件；端点 `00`/`04` 没有 CONTCAR/OSZICAR 属正常，计入 missing）。
  顺带记录：用户 20:33 归档 Ag 的 NEB PATH1/PATH3 时审计是 `saved=- missing=OUTCAR,OSZICAR` —— 因为**运行中的服务还是 2026-09-21 23:24 启动的旧代码**（v0.9.21 的归档改动是 2026-09-22 20:26 才写的），旧逻辑只在 NEB 主目录找 OUTCAR/OSZICAR（映像文件在 `0X/` 里），所以什么都没下。**重启后**若要重新归档已归档的任务，需先"重新打开"再"关闭"（已归档再点关闭返回 409）。
- v0.9.22（commit `fc73101`，已推送 origin/main；2026-09-22 用户："按最好的改，要确保改完系统运行不受影响"）：**隔离数据目录的本地镜像根目录自动落在数据目录内**（修掉"隔离实例把本地镜像写回生产树"的隐患）。
...
- v0.9.21（commit `6bdd049`，已推送 origin/main；2026-09-22 用户对 TODO「发现问题」②③ 说"做吧"）：**归档（关闭任务）时的文件同步扩到六件套 + NEB 按映像目录留档**。
  ...
  ③ 实现都在 `backend/input_state.py`：新增 `ARCHIVE_NEB_SHARED / ARCHIVE_NEB_IMAGE_FILES` 与 `_archive_one_dir()`（按目录批量下、单文件失败只记 missing）；`download_archive_outputs()` 按任务类型分流，返回 `saved / missing / images / image_saved / image_missing / overwrote_draft`；归档后台线程（`jobs._schedule_archive_outputs`）的审计行相应扩展成
     `OK saved=… missing=… images=N img_saved=N img_missing=N [overwrote_draft=…]`。
  ④ **有未生效草稿的本地副本**：归档留档要真实，所以照下，但会记进审计的 `overwrote_draft=`（旧的"有草稿就跳过"只作用于 `build_snapshot`，不影响归档）。
  ⑤ 验证（mock 远端 + 隔离数据目录，真实 HTTP 走 `POST /jobs/tasks/{id}/archive`，10 项全过）：opt 任务把 con2 的六个文件下到 `files/`（内容取自 con2 而不是主目录）；NEB 任务把 `con1` 的 INCAR/KPOINTS 下到 `files/`、`00/01/02` 四个文件下到 `files/0X/`（缺的两个进 `img_missing`）、主目录的 POSCAR 不会混进 `files/` 顶层；审计行是 `saved=CONTCAR,INCAR,KPOINTS,POSCAR,OUTCAR,OSZICAR missing=-` 与 `images=3 img_saved=10 img_missing=2`；真实分支的 bash 列目录片段单独跑过（`00/01/02`、`con1`、目录不存在时输出空）。（顺带：本轮又踩到"隔离测试的 `path_mapping.local_root` 必须是绝对路径"，泄漏出来的空目录已清掉。）
...
- v0.9.12（commit `43d1640`，已推送 origin/main）：**规则支持"动作成功后自动提交作业"接力**（用户："提交作业作为勾选项放在动作后面，因为创建续算完成后一般都会接着提交"）。
  ...
  ④ 验证：后端 9 项（续算成功→自动提交；续算 skipped→不提交；主动作失败→不提交；显式幂等键仍去重；无幂等键可重复执行；规则保存 follow_up_action；后续动作未知/与主动作相同 → 400；手动入口接受 follow_up）+ 前端 5 项（规则表显示接力、弹窗勾选项存在且位于动作之后、编辑时回填、主动作为提交作业时禁用）+ `tsc` + build。
- v0.9.11（commit `dd23209`，已推送 origin/main）：**定时改中文日程交互 + 表单换行 + 定时任务可增删改**。
  ① **界面不再出现调度表达式/专有名词**（用户："为什么是 0 2 * * *；前端不要写 cron 这类专有名词"）：原来的「cron（分 时 日 月 周）」输入框换成中文选择器——
     - 什么时候执行：**按周期重复** / **稍后执行一次**；
     - 重复方式：每隔几分钟 / 每隔几小时 / 每天 / 每周 / 每月（外加"高级：手动填写表达式"给特殊需求）；
     - 具体值：间隔数字 + 分钟/小时、`TimePicker` 选时间（HH:mm）、星期几、每月几号；
     - 旁边实时给出中文预览（如"将按「每天 02:00」执行"），列表里也只显示"每天 02:00 / 周一 08:30 / 每隔 30 分钟 / 30 分钟后执行一次一次"这类人话。
     双向转换在 `src/utils/schedule.ts`（纯函数、已单测）：`scheduleToCron / cronToSchedule / describeSchedule / describeDelay`；认不出的历史表达式归为"自定义"并原样回显，不会被改坏。
  ② **修「触发方式改为定时后四个框超出弹框」**：所有表单行改成可换行（antd `Space wrap` → `flex-wrap: wrap`），弹窗宽度 720→800，字段在小宽度下自动折到下一行。
  ③ **定时任务也能编辑/删除**（用户："定时任务为什么不能删除"）：定时任务表的操作列补齐「编辑」（打开同一个规则弹窗，按时人话回填）与「删除」（带确认，连带清理冷却/计数/下次触发历史）。
  ④ 验证：前端 14 项（人话↔表达式往返 7 项：每天/每周/每月/每隔分钟/每隔小时/自定义/一次性延迟；界面 7 项：定时任务表显示"每天 02:00"且不再出现表达式、行内有编辑与删除、弹窗无 cron 字样、出现中文日程控件与预览、所有表单行 `flex-wrap: wrap`）+ `tsc` + `npm run build`。
- v0.9.10（commit `c8b3ed7`，已推送 origin/main）：**具体对象改成树状分流 + 去掉 job_id 条件字段**。
  ① **任务/组按树选择**（用户："具体任务要做好树状分流，太多了太乱了找不到"）：
     - opt 任务用 `TreeSelect`：**项目 → 组（PATH1（2）/ 独立任务）→ 任务**，只有项目与组节点不可选（纯分流），叶子才是具体任务；
     - 自由能组用 `TreeSelect`：**项目 → 自由能组（含结构数）**；
     - 两者都支持直接输入搜索（按节点标题过滤，任务多时不用翻）、多选、标签折叠（`maxTagCount="responsive"`）。
     树由纯函数 `buildOptTaskTree() / buildFreeEnergyGroupTree()`（`src/utils/ruleTarget.ts`）生成，输入是页面从 `/api/projects` 摊平的任务列表（带 group 元数据）。
  ② **去掉 `job_id` 条件字段**（用户："这个作业号有什么用，作业号不是每次提交都会变吗"）：确认它对规则没有意义（每次 `bsub` 都会拿到新号，规则若按它过滤只会在"恰好是那个号"的瞬间命中），因此从附加条件字段目录与后端 `CONDITION_KEYS` 白名单里一并移除；要定位具体任务请用「任务名（完全匹配）」。
  ③ 验证：前端 14 项（树结构 8 项：项目分流、组/独立任务分支、叶子可选、只含 opt、组带结构数、自由能组树不混 NEB；字段目录 2 项；弹窗交互 4 项：切到 opt 出现树选择、树里有项目节点、展开项目看到组、展开组看到具体任务）+ `tsc` + `npm run build`。
- v0.9.9（commit `8469cd5`，已推送 origin/main）：**规则配置改三级交互（选作用对象 → 选具体对象 → 附加条件也用下拉）**。
