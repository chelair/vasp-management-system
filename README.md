...
当前版本（v0.8.2）：四个核心模块（总览 / 巡检中心 / 作业管理 / 智能报告）+ SSH 连接配置界面；
...
- **改参数不碰远端（v0.8.2）**：编辑器默认只读，点「修改参数」解锁 → 「确认修改」才记为**待生效修改**
  （高亮 + 原值提示 + 可逐项撤销），**只在下次续算时写入新 conN**；续算会在 INCAR 顶部留审计注释，
  POSCAR 永不覆盖（续算 POSCAR 来自 CONTCAR）
- **POSCAR 页（v0.8.2 重构）**：460px 3Dmol 大窗口，可切换**初始 POSCAR / 最新 CONTCAR**，
  **点击选中原子**（金色高亮，多选，为后续固定原子铺路）+ 结构数据（元素组成/原子数/晶格）+
  球棍/空间填充、原子缩放、自动旋转、a/b/c 视角、重置视角
