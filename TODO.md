...
**已完成（v0.9.12，2026-09-21 · 动作成功后自动提交作业接力）**

- [x] 规则新增 `follow_up_action`：弹窗"动作"后面勾选「执行成功后自动提交作业」，列表显示 `续算 → 提交` 接力
- [x] 只有主动作成功才接力（续算 skipped / 失败都不提交）；接力同样受冷却与执行上限约束；手动动作入口也支持 `follow_up`
- [x] 修掉隐式幂等指纹：不再自动生成 `task|action` 永久指纹（它会让同一规则第二天再也跑不起来），只认显式 `idempotency_key`
- [x] 验证：后端 9 项 + 前端 5 项 + tsc/build


**已完成（v0.9.11，2026-09-21 · 定时中文日程交互 + 表单换行 + 定时任务可增删改）**

- [x] 定时配置不再出现调度表达式/专有名词：什么时候执行（周期重复 / 稍后一次）+ 重复方式（每 N 分钟/小时、每天、每周、每月、高级自定义）+ 时间/星期/日期选择 + 中文预览
- [x] 列表里定时任务显示"每天 02:00 / 周一 08:30 / 每隔 30 分钟 / 30 分钟后执行一次"；历史自定义表达式原样保留
- [x] 修「触发方式选定时后字段超出弹框」：表单行改 `Space wrap`（flex-wrap），弹窗加宽到 800
- [x] 定时任务表补「编辑 / 删除」（连带清理冷却与下次触发历史）
- [x] 验证：前端 14 项（人话↔表达式往返 7 + 界面 7）+ tsc/build


**已完成（v0.9.10，2026-09-21 · 具体对象树状分流 + 去掉 job_id 条件字段）**

- [x] opt 任务用「项目 → 组（含结构数）/独立任务 → 任务」树选择，自由能组用「项目 → 自由能组」树选择；均支持搜索与多选
- [x] 去掉 `job_id` 条件字段（每次提交都会变，对规则无意义），前端字段目录与后端白名单同步移除
- [x] 验证：前端 14 项（树结构 8 + 字段目录 2 + 弹窗交互 4）+ tsc/build


**已完成（v0.9.9，2026-09-21 · 规则三级交互：作用对象 → 具体对象 → 附加条件下拉）**

- [x] 选完作用对象后可选**具体项目 / 具体 opt 任务 / 具体自由能组**（可搜索、可多选），生成 `project` / `task_id` / `group_id` 条件
- [x] 定时规则按所选对象反查项目：单个项目 → `scope=project:<名>`，多个 → `scope=all` + 条件过滤
- [x] 附加条件全部改成下拉（字段中文名 + 说明；布尔给是/否、状态与角色给多选、只有任务名/作业号需要手填）
- [x] 后端条件字段白名单：未知字段 400（避免规则静默永不命中）；列表/布尔/单值都支持
- [x] 验证：前端 17 项 + 后端 4 项 + tsc/build


**已完成（v0.9.8，2026-09-21 · 规则改「选作用对象」+ 下架 neb.create）**

- [x] 新建/编辑规则改为先选「作用对象」（项目 / opt 任务 / 自由能组）+「指定项目（可选）」，条件降级为折叠的「附加条件」；编辑时自动反推回填
- [x] 下架 `neb.create` 动作（动作目录 3 个），默认规则与生产数据里的 NEB 规则一并清理
- [x] 验证：后端 7 项 + 前端 15 项（含映射函数单测与动作下拉不含 NEB）+ tsc/build


**已完成（v0.9.7，2026-09-21 · 自动化规则增删改 + 定时"N 分钟后执行一次" + 开关即时生效）**

- [x] 规则 CRUD：`POST/DELETE /api/automation/rules`、`PUT` 支持全字段编辑；前端「新建规则」+ 行内「编辑/删除」
- [x] 定时开关点击即时生效（规则表与定时任务表状态同步，不再需要刷新页面）
- [x] 规则表只显示条件规则，定时规则只在定时任务表（两表按触发类型分开）
- [x] 新增定时类型 `mode=after`：N 分钟后执行一次（一次性 + 重新计时 + 已执行状态展示）
- [x] 修 `store.delete_rule` 的 flock 自锁（会把服务卡死）；`AUTOMATION_TICK_SECONDS` 可调 tick
- [x] 验证：50 项断言（规则 CRUD 17 / after 13 / 前端 10 / 守卫回归 10）+ tsc/build


**已完成（v0.9.6，2026-09-20 · 自动/定时执行动作系统）**

- [x] 五层架构：事件总线（巡检只发事件）/ 规则层（`config/rules/*.json`，改完立即生效）/ 调度层（队列 + 任务锁 + 冷却 + 上限 + 幂等 + 熔断 + 全局暂停）/ 执行层（统一入口 `/api/actions/*`，复用 `jobs.core_*`）/ 审计层（`actions.jsonl` 五态）
- [x] 四个动作：`task.continuation`（按 `action=created` 判定成功）/ `task.submit`（五件套 + NEB 映像检查，支持 dry_run）/ `neb.create`（长动作，run_id 轮询）/ `frac.create`（长动作）
- [x] 定时触发：零依赖 5 段 cron（未引入 APScheduler）+ 持久化 next_run + 改 cron 立即重算；示例规则 4 条（默认启用）
- [x] 前端「自动化」页（仅 admin）：全局开关 / dry_run / 定时开关 + 定时任务表 + 规则表 + 决策日志 + 运行记录
- [x] 验证：36 项自动化断言（含 dry_run、冷却、上限、幂等、手动接管、暂停、长动作轮询、cron 重载、五态审计、403）+ 11 项 jobs 核心抽取回归 + tsc/build
...
### 13. 智能体闭环：巡检 → 判断 → 执行（规则化自动执行已在 v0.9.6 落地）
...
### 16. 接力动作的兜底（2026-09-21 事故复盘，**未实现**）

> 来源：排查"续算建了 conN 却没提交"时发现主因是字段保存 bug（v0.9.13 已修），但也暴露了接力语义上的两个缺口。

1. **接力提交被拦后不会补**：`follow_up_action` 只在主动作成功的那一次投递。如果这次被 `blocked`
   （冷却中 / `MANUAL_HOLD_SECONDS=300` 内有人手动操作过该任务 / 任务级锁忙），动作不会重试；
   而续算成功后**父任务状态会回落成 `pending`**，规则的 `status` 条件（通常写 completed/zombied/unconverged）
   就不再命中，直到该 conN 自己跑完被巡检改回状态 —— 也就是"续算建了但一直没提交"。
   建议：① 接力 blocked 时落一条"待补"记录，下一轮巡检事件优先补投（同样受冷却/上限约束）；
   或 ② 增加一个条件字段（如 `pending_continuation`：父任务下存在未提交的 conN），让规则能直接命中。
2. **规则"静默不匹配"不易察觉**：父任务变 `pending` 后，规则既不报错也不命中，页面上看不出"这条规则
   此刻没人可作用"。建议决策日志/规则列表补「最近命中时间 + 最近一次 skip 原因」，一眼能看出规则是否还活着。

### 17. 「复制 INCAR 参数到其他作业」是否要支持"同时写远端"（2026-09-21 待用户确认）

> 背景：用户问"点击后是不是直接远端运行 cp"，排查后确认现状**只写目标任务的待生效草稿、不碰远端任何文件**
> （v0.9.18 有实测证据：远端 INCAR 哈希不变、审计里没有远端命令；顺便把容易误解的按钮文案
> 「同步到 N 个作业」改成「记为 N 个作业的待生效修改」）。

> 进展（v0.9.19）：用户报"点击后没反应"的根因已修——自由能/NEB **组页面**里 `selectedTask` 是 `null`，
> 来源作业取不到就静默 return（弹窗不关、无提示、零请求）；现在打开弹窗时显式记住来源作业、取不到会明确报错。
> 下面"是否同时写远端"的选择仍然待定。

**待定选项**

1. 维持现状（推荐先这样）：只写草稿，目标作业下次续算时应用；要立即生效就在目标作业上点「同步到远端」。
2. 加一个「同时同步到远端」勾选项：勾上后对每个目标任务立刻调用 `upload-incar`
   （`modify_incar(远端旧 INCAR, params)` 合并 + 旧文件备份 `old_INCAR`，**不是整文件覆盖**）。
   风险：会直接改动别的作业正在使用的输入文件；建议要求"目标作业不在 queued/running"才允许，
   并把结果改成逐个汇报成功/失败（现在 `Promise.all` 会静默吞掉单项失败）。

两种做法的差异只在"是否立刻写远端"，草稿链路一样；用户确认后再动代码。
